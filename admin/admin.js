...
function setMessage(text, type=''){ const el=$('#loginMessage'); el.textContent=text||''; el.className='message '+type; }
function showApp(){ $('#authPanel').classList.add('hidden'); $('#appPanel').classList.remove('hidden'); $('#logout').classList.remove('hidden'); if(adminUser) $('#adminIdentity').textContent=`Logged in as ${adminUser.email}`; loadStats(); }
function showAuth(){ $('#authPanel').classList.remove('hidden'); $('#appPanel').classList.add('hidden'); $('#logout').classList.add('hidden'); }
function switchAuth(mode){ const login=mode==='login'; $('#loginTab').classList.toggle('active', login); $('#signupTab').classList.toggle('active', !login); $('#loginForm').classList.toggle('active', login); $('#signupForm').classList.toggle('active', !login); setMessage(''); }
...
$('#loginForm').addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    token=data.token; adminUser=data.admin;
...
$('#logout').onclick=async()=>{ try{ if(token) await api('/admin/logout',{method:'POST'}); }catch(_){} localStorage.removeItem('galaxyAdminToken'); token=''; adminUser=null; location.reload(); };
...
async function verifySession(){ if(!token) return showAuth(); try{ const data=await api('/admin/me'); adminUser=data.admin; showApp(); }catch(e){ localStorage.removeItem('galaxyAdminToken'); token=''; showAuth(); } }
...
$('#loadProducts').onclick=loadProducts; $('#refreshBtn').onclick=()=>{loadStats(); if(document.querySelector('[data-view="products"].active')) loadProducts();};
$('#newProduct').onclick=()=>{ $('#productForm').reset(); $('#editId').value=''; };
$('#productForm').addEventListener('submit',async(e)=>{e.preventDefault();const body={id:$('#editId').value,name:$('#editName').value,price:$('#editPrice').value,tab:$('#editTab').value,department:$('#editDepartment').value,category:$('#editCategory').value,upc:$('#editUpc').value,image_url:$('#editImage').value}; const data=await api('/admin/products',{method:'PUT',body:JSON.stringify(body)}); $('#editId').value=data.id; await loadProducts(); await loadStats();});
$('#uploadImageBtn').onclick=async()=>{try{const file=$('#uploadImageFile').files[0]; if(!file) throw new Error('Choose an image file.'); const id=$('#uploadProductId').value.trim(); if(!id) throw new Error('Choose/paste product ID.'); const fd=new FormData(); fd.append('image',file); fd.append('productId',id); const res=await fetch(API+'/admin/images/upload',{method:'POST',headers:{Authorization:`Bearer ${token}`},body:fd}); const data=await res.json(); if(!res.ok) throw new Error(data.error||'Upload failed'); $('#uploadMessage').textContent='Uploaded and attached.'; await loadProducts(); await loadStats();}catch(e){$('#uploadMessage').textContent=e.message;}};
